...
const InfoTicket = ({ TicketData }) => {
  ...
  const [showDialog, setShowDialog] = useState(false); // Trạng thái để hiển thị cảnh báo xóa
  const [ticketId, setTicketId] = useState(null); // State lưu ticketId

  let ticketTime = TicketData[1]; // Giả sử TicketData[1] là timestamp

  // Hàm mở modal và lưu ticketId
  const cancelTicketDg = async (ticketId) => {
    setTicketId(ticketId); // Lưu ticketId vào state
    setShowDialog(true); // Mở cảnh báo
  };

  const confirmTicketDialog = async () => {
    await handleCancelClick(ticketId);
    setShowDialog(false); // Đóng cảnh báo
  };

  const cancelTicketDialog = async () => {
    setShowDialog(false); // Đóng cảnh báo mà không xóa
    notificationWithIcon('info', 'Đã hủy quá trình hủy vé', '');
  };
  ...
  return (
    <>
            ...
            <div className="col-md-3 text-center d-flex flex-column justify-content-center align-items-center">
              ...
              {showCancelButton ? (
                <button className="btn btn-link btn-link-red"
                  onClick={() => cancelTicketDg(TicketData[9])}
                >
                  Cancel Ticket
                </button>
      ...
      <NotificationDialog
        ...
        isOpen={showDialog}
        onClose={cancelTicketDialog}
        type="warning"
        onConfirm={confirmTicketDialog}
        onCancel={cancelTicketDialog}
